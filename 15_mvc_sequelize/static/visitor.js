  ...
  axios({
    ...
    .then((res) => {
      ...
      <tr id="tr_${id}">
          ...
          <td>${comment}</td>
          <td>
            <button onclick="editVisitor('<%= data[i].id %>')">수정</button></td>
          <td><button onclick="deleteVisitor(this,'<%= data[i].id %>')">
              삭제
            </button></td>
        </tr>`;
...
function editDo(id) {
  ...
  axios({
    ...
    .then((res) => {
      ...
      console.log(tr.childern);
      const childern = tr.childern; //[td,td,td,td,td]
